...
public interface UserMapper {
    ...
    @Mapping(target = "id", source = "user.id") // Chỉ định ánh xạ từ user.id
    @Mapping(target = "profile", source = "profile") // Ánh xạ profile từ đối tượng Profile
    @Mapping(
            target = "roles",
            expression = "java(mapGlobalRoleToRoles(user.getGlobal_role()))") // Ánh xạ global_role thành roles
    ...
    default Set<RoleResponse> mapGlobalRoleToRoles(GlobalRole globalRole) {
        ...
        return Collections.singleton(
                new RoleResponse(globalRole.getName())); // Chuyển GlobalRole thành RoleResponse và cho vào Set
    }
}
