...
public class UserService {
    ...
    public UserResponse createUser(UserProfileCreationRequest userProfileCreationRequester) {
        ...
        // set role
        if (userProfileCreationRequester.getGlobal_role_id() == null) {
            userProfileCreationRequester.setGlobal_role_id(3);
        }
        GlobalRole globalRole = globalRoleRepository
                .findById(userProfileCreationRequester.getGlobal_role_id())
                .orElse(null);
    ...
    public List<UserResponse> getAllUsers() {
        ...
        return users.stream()
                .map((user -> {
                    Profile profile = profileReposity
                            .findProfileWithUserByUserId(user.getId())
                            .orElse(null);
                    return userMapper.toUserResponse(user, profile);
                }))
                .toList();
    ...
    public UserResponse updateUser(Integer userId, UserProfileCreationRequest userProfileCreationRequest) {
        ...
        User user = userRepository.findById(userId).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
        Profile profile = profileReposity
                .findProfileWithUserByUserId(userId)
                .orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));

        // update user
        user.setEmail(
                user.getEmail().equals(userProfileCreationRequest.getEmail())
                        ? user.getEmail()
                        : userProfileCreationRequest.getEmail());
        user.setPassword(
                user.getPassword().equals(userProfileCreationRequest.getPassword())
                        ? user.getPassword()
                        : userProfileCreationRequest.getPassword());
        user.setUpdated_at(Timestamp.from(Instant.now()));

        GlobalRole globalRole = globalRoleRepository
                .findById(userProfileCreationRequest.getGlobal_role_id())
                .orElse(null);
        ...
        profile.setFull_name(
                profile.getFull_name().equals(userProfileCreationRequest.getFull_name())
                        ? profile.getFull_name()
                        : userProfileCreationRequest.getFull_name());
        profile.setAddress(
                profile.getAddress().equals(userProfileCreationRequest.getAddress())
                        ? profile.getAddress()
                        : userProfileCreationRequest.getAddress());
        profile.setPhone(
                profile.getPhone().equals(userProfileCreationRequest.getPhone())
                        ? profile.getPhone()
                        : userProfileCreationRequest.getPhone());
        profile.setBirthday(
                profile.getBirthday().equals(userProfileCreationRequest.getBirthday())
                        ? profile.getBirthday()
                        : userProfileCreationRequest.getBirthday());
        profile.setGender(
                profile.getGender().equals(userProfileCreationRequest.getGender())
                        ? profile.getGender()
                        : userProfileCreationRequest.getGender());
